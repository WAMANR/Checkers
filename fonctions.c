/**
 * \file fonctions.c
 * \brief Fonctions manipulation du checkers.
 * \author MONTAIGU Theo, ROUICHI Adil, MOMPI Emmanuel, BONNEROT Thomas
 * \version 1.0
 * \date 09 février 2020
 *
 * Contient les fonctions qui manipule le checkers et ses données.
 *
 */
#include "checkers.h"
...
int Win(){
  ...
  // Elle prend rien parametre
  // Elle retourne un entier en fonction de la situation de victoire
...
void TablPiecePossible(){
  ...
  // Elle  regarde les pièces possible dans le tableau
  // Elle prend rien en paramètre
  // ELle ne retourne rien  
...
void TablMovePossible(){
  ...
  // ELle regarde les mouvements possibles dans le tableau
  // Elle ne pend rien en parametre
  // Elle retourne rien
...
void InitBoard(){
  ...
  // Elle prend rien en paramètre
  // ELle ne retourne rien, c'est un void
...
tree * BuildTree(tree * t, checkers game, int x, int y, int prof){

  // Fonction qui fabrique l'arbre, qui regarde le coup le plus optimal
  // Elle prend en parametre la profondeur, les coordonnés, la structure tree et checkers
  // Elle renvoie un arbre  
  ...
  printf("AAA\n");
      ...
      t->fils[t->nbfils] = BuildTree(t->fils[t->nbfils],game,x+2,y+2,prof+1);
      t->nbfils+=1;
  ...
  while(game.board[y+i+j][x+i+j]==0 && y+i+j<10 && x+i+j<10){
    ...
    t->fils[t->nbfils] = BuildTree(t->fils[t->nbfils],game,x+i+j,y+i+j,prof+1);
    t->nbfils+=1;
    //On remet le board a l etat precedent:
...
tree * MakeTree(tree * t, checkers game){
  ...
  for(j=0; j<10; j++){
    for(i=0; i<10; i++){
      if(game.board[j][i]==game.player || game.board[j][i]==game.player+2){
        ...
        t->fils[t->nbfils] = BuildTree(t->fils[t->nbfils],game,i,j,1);
  t->nbfils+=1;
  //printf("FILS:%d\n",t->nbfils);
      }
    }
...
int ChoicePiecePossibleCpy(int xxx, int yyy){
